...
typedef  struct  _CAN_MSG
{
    ...
    unsigned int    TimeStamp;    ///<接收到信息帧时的时间标识，从CAN 控制器初始化开始计时。单位为微秒
...
typedef struct _CAN_INIT_CONFIG
{
    ...
    unsigned char   CAN_Mode;   ///<CAN工作模式，0-正常模式，1-环回模式，2-静默模式，3-静默环回模式，bit6-使能错误帧接收，bit7-使能内部终端电阻
 ...
 *@{
*/
#define CAN_MODE_NORMAL          0       ///<正常模式，需要外接CAN节点才能正常收发数据
#define CAN_MODE_LOOP_BACK       1       ///<回环模式，无需外接CAN节点就可以实现自发自收
#define CAN_MODE_EN_RESISTOR     0x80    ///<使能120Ω终端电阻
#define CAN_MODE_REV_ERROR       0x40    ///<使能错误帧接收
/** @} */
