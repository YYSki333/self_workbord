 ...
 * @retval <0 函数调用失败
 */
int WINAPI  LIN_EX_MasterSendMsg(int DevHandle, unsigned char LINIndex, LIN_EX_MSG* pInMsg, LIN_EX_MSG* pOutMsg, unsigned int MsgLen);
...
/**
 * @brief  设置从机响应数据表，可以通过该函数设置某个ID响应多帧数据，该函数为非阻塞函数
 * @param  DevHandle 设备号，通过调用 @ref USB_ScanDevice 获取
 * @param  LINIndex LIN通道号，0-LIN1，1-LIN2，2-LIN3，3-LIN4
 * @param  PID 需要响应数据的ID
 * @param  DLC 单帧发送数据字节数，不能大于8
 * @param[in] pData 需要响应的数据
 * @param  DataLen 响应数据字节数
 * @param  RunTimes 数据响应次数，如果设置为0xFFFFFFFF则表示当前数据连续响应
 * @return 函数执行状态
 * @retval 0 函数调用成功
 * @retval <0 函数调用失败
 */
int WINAPI  LIN_EX_SetSlaveRespData(int DevHandle, unsigned char LINIndex, unsigned char PID, unsigned char DLC, unsigned char* pData, unsigned int DataLen, unsigned char CheckType, unsigned int RunTimes);

/**
 * @brief  设置某个ID停止响应数据
 * @param  DevHandle 设备号，通过调用 @ref USB_ScanDevice 获取
 * @param  LINIndex LIN通道号，0-LIN1，1-LIN2，2-LIN3，3-LIN4
 * @param  PID 需要停止响应数据的ID
 * @return 函数执行状态
 * @retval 0 函数调用成功
 * @retval <0 函数调用失败
 */
int WINAPI  LIN_EX_StopSlaveResp(int DevHandle, unsigned char LINIndex, unsigned char PID);
