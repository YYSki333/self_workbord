...
* @section   内容简介
* 本文档为USB2XXX库里面的函数说明文档，对设备进行二次开发的所有接口函数都包含在里面，比如USB转CAN，USB转LIN，USB转SENT，USB转KWP2000，USB转IIC，USB转SPI，USB转PWM等功能。不管二次开发是使用的什么语言，调用的接口函数都是一样的，所以该文档适用于任何二次开发参考，只是不同的语言，其函数调用形式有所不同。
* 
* @section   用法描述 
* -# 首先将对应平台的库文件全部复制到程序工程目录下，USB2XXX可能会依赖libusb和其他库，都需要复制过去
* -# 将不同语言平台下的接口函数文件也复制到工程里面，然后加入到工程中后，程序里面直接调用即可
* -# 库函数使用范例程序源码和最新SDK库下载：<a href="https://gitee.com/toomoss/usb2can_lin_pwm_example.git">https://gitee.com/toomoss/usb2can_lin_pwm_example.git</a>
* -# 各个模块函数说明文档：<a href="http://www.toomoss.cn/api/modules.html">http://www.toomoss.cn/api/modules.html</a>
*
* @section   版本记录 
* <table>
* <tr><th>日期        <th>版本   <th>作者    <th>描述</tr>
* <tr><td>2023/8/24 <td>1.1    <td>wdluo  <td>创建初始版本 </tr>
* <tr><td>2025/9/1 <td>1.2    <td>wdluo  <td>增加LDF,DBC文件解析函数说明 </tr>
* <tr><td>2026/7/8 <td>1.3    <td>wdluo  <td>增加KWP2000总线接口函数说明 </tr>
* </table>
...
typedef struct _IP_CONFIG
{
    ...
    unsigned char   GW_ADDR[4];///<网关
    unsigned char   StaticIP;  ///<0-动态获取IP，1-静态IP
    unsigned char   Res0;
    unsigned char   Res1;
    unsigned char   Res2;
}IP_CONFIG;
 ...
 * @brief  检测设备是否合法
 * @param  DevHandle 设备号，通过调用 @ref USB_ScanDevice 获取
 * @param  pKeyBuf 输入秘钥字符串
 * @param[out]  pKeyOutBuf 输出秘钥字符串
 * @return 设备检测状态
 * @retval 0 检测失败
 * @retval 1 检测成功
 */
...
/**
 * @brief  通过USB口设置设备IP地址，只有带网口的设备才支持此操作
 * @param  DevHandle 设备号，通过调用 @ref USB_ScanDevice 获取
 * @param  pConfig IP地址信息结构体指针
 * @return IP配置状态
 * @retval 0 配置失败
 * @retval 1 配置成功
 */
...
/**
 * @brief  通过USB口获取设备IP地址信息，只有带网口的设备才支持此操作
 * @param  DevHandle 设备号，通过调用 @ref USB_ScanDevice 获取
 * @param  pConfig IP地址信息结构体指针
 * @return IP配置状态
 * @retval 0 配置失败
